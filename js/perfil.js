...
(function () {
    ...
    fotoInput.addEventListener('change', function () {
        ...
        reader.onload = function (e) {
            ...
            showToast('info', 'Vista previa lista', 'La nueva fotografía se aplicará al guardar los cambios.');
        };
    ...
    $('#btnFooterGuardar').addEventListener('click', () => {
        formPerfil.requestSubmit ? formPerfil.requestSubmit() : formPerfil.dispatchEvent(new Event('submit', { cancelable: true }));
    });

    $('#btnFooterPassword').addEventListener('click', () => {
        formSeguridad.requestSubmit ? formSeguridad.requestSubmit() : formSeguridad.dispatchEvent(new Event('submit', { cancelable: true }));
    });

    $('#btnFooterCancelar').addEventListener('click', () => {
        openConfirm(
            'Cancelar cambios',
            '¿Deseas descartar los cambios no guardados y restaurar los valores actuales?',
            () => {
                formPerfil.reset();
                formSeguridad.reset();
                strengthFill.style.width = '0%';
                ...
                showToast('info', 'Cambios descartados', 'Se restauraron los valores anteriores.');
            }
        );
    });

    $all('#btnFooterCerrarSesion, #btnTopCerrarSesion').forEach((btn) => {
        if (!btn) return;
