...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    try {
        $stmtUltimo = $conn->prepare("UPDATE usuario SET ultimo_acceso = NOW() WHERE id_usuario = ?");
        if ($stmtUltimo) {
            $stmtUltimo->bind_param('i', $usuario['id_usuario']);
            $stmtUltimo->execute();
            $stmtUltimo->close();
        }
    } catch (\Throwable $e) {
        // No detiene el login si la columna ultimo_acceso aun no existe
        // (falta correr migracion_perfil.sql). Ver api/error.log si aplica.
    }
